...
export function PosDashboardPanel({
  ...
  error?: string | null;
  onNavigate: (view: PosView, query?: Record<string, string>) => void;
}) {
  ...
  const selectedCounter =
    tags.find((tag) => tag.id === selectedTagId) ?? tags[0] ?? null;
  const selectedReceipt = selectedCounter
    ? latestByTagId[selectedCounter.id] ??
      receipts.find((receipt) => receipt.tag_id === selectedCounter.id) ??
      null
    : null;
  ...
  return (
    <div className="h-full overflow-y-auto bg-cream">
      ...
      <section className="mb-6 px-6 pt-2">
      ...
      {error ? (
        <p className="mx-6 mb-6 rounded-full bg-red-50 px-4 py-2 text-sm font-semibold text-red-700">
          {error}
        </p>
      ) : null}

      {awaitingReceipts.length > 0 ? (
        <section className="mx-6 mb-6 rounded-[16px] border border-amber-200 bg-amber-50 p-4">
      ...
      <section className="mx-6 mb-6 grid grid-cols-[1.4fr_1fr_1fr_1fr] gap-4">
        <StatCard
          dark
          label="Total Revenue"
        ...
        <StatCard
          ...
          trend={`${tags.length} ready for NFC`}
        ...
        <div className="grid grid-cols-[repeat(auto-fill,minmax(220px,1fr))] gap-4">
          {tags.length > 0 ? (
            tags.map((tag) => (
              <CounterCard
                key={tag.id}
                latest={latestByTagId[tag.id]}
                tag={tag}
                onSelect={() => onNavigate("dashboard", { tag: tag.id })}
              />
            ))
          ) : (
      ...
      <section className="mx-6 mb-8 overflow-hidden rounded-[16px] border border-line bg-white shadow-soft">
